...
const create_html_string = _ =>(`
    <div class='rc-container' id='container'>
        <div class='headers'>
            <h1 id='title'>Payments to Russia for fossil fuels</h1>
            <div class='subtitle' id='subtitle'>Since 24 February 2022</div>
        </div>
        ${
            !URL_SETTINGS.eu_only ?
            `<div class='section'>
                <div id='GLB'>
                    <div class='head'>
                        <h2>
        ...
        <div class='caption'>
            <p id='caption'>Source: CREA analysis. See methodology <a href="https://crea.shinyapps.io/russia_counter/?tab=methodology" target='_blank'>here</a></p>
        </div>
    </div>`);
...
const update_ui = (counter_data)=>{
    ...
    for (let data of counter_data) {
        ...
        document.getElementById(`${data.ui_identifier}_total_eur`).textContent = Math.floor(data.total_eur_updated).toLocaleString('en-US');
        document.getElementById(`${data.ui_identifier}_oil_eur`).textContent = Math.floor(data.oil_eur_updated / 1000000).toLocaleString('en-US');
        document.getElementById(`${data.ui_identifier}_coal_eur`).textContent = Math.floor(data.coal_eur_updated / 1000000).toLocaleString('en-US');
        document.getElementById(`${data.ui_identifier}_gas_eur`).textContent = Math.floor(data.gas_eur_updated / 1000000).toLocaleString('en-US');
        document.getElementById(`${data.ui_identifier}_oil_percentage`).textContent = floor_oil.toLocaleString('en-US');
        document.getElementById(`${data.ui_identifier}_gas_percentage`).textContent = floor_gas.toLocaleString('en-US');
        document.getElementById(`${data.ui_identifier}_coal_percentage`).textContent = ceil_coal.toLocaleString('en-US');
    }
}
...
const update_text_of_identifier = (identifier, text)=>{
    document.querySelectorAll(identifier).forEach(elem => elem.textContent = text)
}

const ui_setup = async(container) => {
    let counter_container = document.getElementById(container)
    if(!counter_container){
        console.log('define container element')
        return;
    }
    counter_container.innerHTML = create_html_string(); 
